...
export function getDefaultPromptConfig(contentType?: string): PromptConfig {
  const config = structuredClone(DEFAULT_PROMPT_CONFIG);
  if (contentType) {
...
export function validatePromptConfig(
  ...
  if (typeof c.version !== "number") errors.push("Missing or invalid 'version' (must be a number).");

  const sections = ["global", "planning", "keyframes", "motion", "overlays", "metadata", "audio", "pipeline"];
  for (const section of sections) {
    if (c[section] !== undefined && typeof c[section] !== "object") {
      errors.push(`'${section}' must be an object if present.`);
    }
  }

  if (c.global) {
    if (c.global.rules !== undefined && !Array.isArray(c.global.rules))
  ...
  if (c.keyframes) {
    for (const field of ["composition_rules", "continuity_rules"]) {
      if (c.keyframes[field] !== undefined && !Array.isArray(c.keyframes[field]))
        errors.push(`'keyframes.${field}' must be an array.`);
    }
  }
  if (c.motion) {
    for (const field of ["camera_rules", "motion_rules"]) {
      if (c.motion[field] !== undefined && !Array.isArray(c.motion[field]))
        errors.push(`'motion.${field}' must be an array.`);
    }
  }
