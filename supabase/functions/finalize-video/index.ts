  ...
  try {
    ...
    if (forceFbImagePost) {
      ...
      try {
        ...
        const captionResult = await callText({ messages: [{ role: "system", content: fbSystemPrompt }, { role: "user", content: imageDesc }], model: MODELS.TEXT_CHEAP, max_tokens: 1024 });
        ...
        if (isFutureScheduledDate(fbRunMeta.publish_scheduled_date, fbRunMeta.publish_timezone)) {
          ...
          await log("info", `FB image: scheduled_date ${fbRunMeta.publish_scheduled_date} is in the past — posting immediately.`);
        }
            ...
            if (RENDI_API_KEY) {
              ...
              for (let i = 0; i < imageOverlays.length; i++) {
                ...
                if (resScale !== 1) {
                  ...
                  filterParts.push(
                    `[${currentVideoLabel}][${imageInputIdx}:v]overlay=enable='between(t\\,${startSec.toFixed(1)}\\,${endSec.toFixed(1)})':${pos}[${outLabel}]`
                  );
                }
              ...
              for (const textOv of textOverlays) {
                ...
                for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
                  const lineText = escapeFFmpegDrawtextText(lines[lineIdx]);
