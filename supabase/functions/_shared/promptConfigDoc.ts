// Auto-generated from docs/PROMPT_CONFIG_REFERENCE.md
// Update by re-running the generation script
export const PROMPT_CONFIG_DOCUMENTATION = `# Prompt Config JSON — Complete Reference for AI Agents
...
\`\`\`jsonc
{
  ...
  "voiceover": {
    ...
    "voice_id": "fIGaHjfrR8KmMy0vGEVJ", // ElevenLabs voice ID (default: George)
...
- **Use \`content_mode: "ai_generated"\` with a \`content_prompt\`** for dynamic text that adapts to whatever the AI planner decides to create. The prompt should describe the _type_ of text you want, not the literal text.
...
> **Important**: The \`title_prompt\`, \`description_prompt\`, and \`hashtag_prompt\` fields are _additional guidance_ that augments the built-in platform-specific best practices — they do NOT replace them. The system already knows the optimal format for each platform.
...
The \`instruction\` field is critical — it tells the planner _how_ to use the memory. Be specific and actionable:
...
5. **AI-generated overlay text receives full context** — the AI knows the concept prompt AND what the planner decided to create for each scene. Write \`content_prompt\` as an instruction for _what kind of text to write_, not the literal text.
...
11. **Metadata is generated per-platform automatically.** The system produces uniquely optimized titles, descriptions, and hashtags for Instagram, TikTok, YouTube Shorts, and Facebook — following each platform's 2026 best practices. The \`title_prompt\`, \`description_prompt\`, and \`hashtag_prompt\` fields in \`metadata\` are _additional guidance_ that augments (not replaces) the built-in platform rules.
